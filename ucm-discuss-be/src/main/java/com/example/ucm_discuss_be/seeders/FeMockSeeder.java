...
public class FeMockSeeder implements CommandLineRunner {
        ...
        public void run(String... args) {
                if (threadRepository.findAll().stream()
                                .anyMatch(thread -> "Sharing: Pengalaman styling pakai Material 3 di React Native"
                                                .equals(thread.getTitle()))) {
                        return;
                }
        ...
        private List<CourseModel> ensureCourses() {
                ...
                return courseRepository.findAll().stream()
                                .filter(course -> existingCourses.containsKey(course.getCourse_code())
                                                || desiredCourses.stream()
                                                                .anyMatch(desired -> desired.getCourse_code()
                                                                                .equals(course.getCourse_code())))
        ...
        private CommentModel ensureComment(
                ...
                return commentRepository.findAll().stream()
                                .filter(comment -> content.equals(comment.getContent()) && comment.getThread() != null
                                                && thread.getId().equals(comment.getThread().getId()))
                                .findFirst()
                                .orElseGet(() -> {
        ...
        private void ensureReply(CommentModel parentComment, CommentModel replyComment) {
                boolean exists = replyRepository.findAll().stream().anyMatch(reply -> reply.getParent_comment() != null
        ...
        private void ensureThreadVote(UserModel user, ThreadModel thread) {
                boolean exists = userVotesThreadRepository.findAll().stream().anyMatch(vote -> vote.getUser() != null
                                && vote.getThread() != null
        ...
        private void ensureCommentVote(UserModel user, CommentModel comment) {
                boolean exists = userVotesCommentRepository.findAll().stream().anyMatch(vote -> vote.getUser() != null
        ...
        private void ensureViewedThread(UserModel user, ThreadModel thread, LocalDateTime viewedAt) {
                boolean exists = userViewedThreadRepository.findAll().stream().anyMatch(view -> view.getUser() != null
                                && view.getThread() != null
        ...
        private void ensureNotification(UserModel user, CommentModel comment, boolean isRead) {
                boolean exists = notificationRepository.findAll().stream()
                                .anyMatch(notification -> notification.getUser() != null
